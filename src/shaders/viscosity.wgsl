...
@group(0) @binding(3) var<storage, read> cellStartIndices: array<u32>;
...
@group(0) @binding(7) var<storage, read_write> viscosities: array<vec4<f32>>;
...
fn cs_main(@builtin(global_invocation_id) gid: vec3<u32>) {
  let index = gid.x;
  if (index >= sp.particleCount) { return; }
        ...
        loop {
          ...
          if (k != index) {  
            ...
            if (r2 < h2) {
              ...
              let r    = 1.0 / rinv;
              let lapW = pp.viscosity * (pp.h - r);   // viscosity = 45/(π h^6)

              let rhoj = max(densities[k], 1e-8);
              let vj   = velocities[k].xyz;

              let invRhoAvg = 2.0 / (rhoi + rhoj);
              fi += pp.viscosityMu * pp.mass * (vj - vi) * invRhoAvg * lapW;
            }
          }
          k = k + 1u; // increment k
        }
